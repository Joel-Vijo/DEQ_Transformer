...
def anderson_solver(f, z_init,y, m=5, lam=1e-4, max_iter=50, tol=1e-5, beta=1.0):
    x0 = z_init
    x0_new,x1 = f(x0,y)
    x1_new,x2 = f(x0_new,y)
    # print(x0.shape)
    # print(y.shape)
    x0=torch.t(x0)
    x0_new=torch.t(x0_new)
    x1_new=torch.t(x1_new)
    y=torch.t(y)
    # X = torch.cat([torch.stack([x0, x0_new]), torch.zeros((3, *x0.shape))])
    # F = torch.cat([torch.stack([x0_new, x1_new]), torch.zeros((3, *x0.shape))])
    x0=x0.detach().numpy()
    x0_new=x0_new.detach().numpy()
    x1_new=x1_new.detach().numpy()

    # print(x0.shape)
    # print(x0_new.shape)
    xk=np.zeros((16,100))
    outputs = torch.zeros(16,100,119547)
    for i in range(16):
        X = jnp.concatenate([jnp.stack([x0[i], x0_new[i]]), jnp.zeros((3, *jnp.shape(x0[i])))])
        F = jnp.concatenate([jnp.stack([x0_new[i], x1_new[i]]), jnp.zeros((3, *jnp.shape(x0[i])))])
        ...
        for k in range(2, max_iter):
            ...
            # print(alpha.shape)
            # print(F[:n].shape)
            xk[i] = beta * jnp.dot(alpha, F[:n]) + (1-beta) * jnp.dot(alpha, X[:n])
            xk_exp = np.expand_dims(xk[i],1)
            X = X.at[k % 5].set(xk[i])
            # print("xk_exp",torch.from_numpy(xk_exp).shape)
            y_exp=y[i].unsqueeze(1)
            # print("Y",y_exp.shape)
            xk_exp=torch.from_numpy(xk_exp)
            xk_exp=xk_exp.type(torch.LongTensor)
            out_exp,output=f( xk_exp,y_exp)
            outputs[i]=output.squeeze(1)
            out_exp=np.squeeze(np.array(out_exp))
            # print(out_exp.shape)
            # print(F[k % 5].shape)
            F = F.at[k % 5].set(out_exp)

            res = jnp.linalg.norm(F[k % 5] - X[k % 5]) / (1e-5 + jnp.linalg.norm(F[k % 5]))
            if res < tol:
                break
    print("XK",outputs.shape)
    return outputs